...
export const TribunalSelector = forwardRef<HTMLButtonElement, TribunalSelectorProps>(
  (
    ...
    const borderClasses = hasError
      ...
      : 'border-slate-300 hover:border-slate-400 focus:border-brand focus:ring-2 focus:ring-brand/25 dark:border-slate-800 dark:hover:border-slate-700 dark:focus:border-brand dark:focus:ring-brand/20';
            ...
            <div className="relative w-full">
              ...
              <ListboxButton
                ...
                className={`
                  ...
                  rounded-xl border bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100
                  ...
                  ${borderClasses}
                  ${open ? 'border-brand ring-2 ring-brand/20 dark:border-brand dark:ring-brand/20' : ''}
                  ${className}
                `}
              >
                ...
                {/* Conteúdo selecionado formatado com destaque Primary */}
                    ...
                    isCustomSelected ? (
                      <div className="flex items-center gap-2 truncate">
                        ...
                        {selectedOption.sigla && (
                          <span className="shrink-0 rounded-md bg-brand/15 px-1.5 py-0.5 text-[11px] font-mono font-bold text-brand dark:bg-brand/20 dark:text-brand border border-brand/25 dark:border-brand/30">
                            {selectedOption.sigla}
                          </span>
                        )}
                ...
                <div className="absolute right-2.5 flex items-center gap-1">
                  {clearable && isCustomSelected && !disabled && (
                    <button
                      type="button"
                      onClick={handleClear}
                      className="rounded-lg p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-600 dark:hover:bg-slate-800 dark:hover:text-slate-200 focus-visible:outline-hidden focus-visible:ring-1 focus-visible:ring-brand transition-colors"
                  ...
                  <ChevronDown
                    className={`h-4 w-4 text-slate-400 transition-transform duration-200 ease-out ${
                      open ? 'rotate-180 text-brand dark:text-brand' : 'group-hover:text-slate-600 dark:group-hover:text-slate-200'
                    }`}
                    aria-hidden="true"
                  />
                </div>
              ...
              >
                <ListboxOptions
                  static
                  className="absolute z-50 mt-1.5 max-h-72 w-full overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl dark:border-slate-800 dark:bg-slate-950 focus:outline-hidden"
                >
                  {/* Campo de pesquisa interna quando searchable=true */}
                  {searchable && normalizedOptions.length > 5 && (
                    <div className="border-b border-slate-100 p-2 bg-slate-50/90 dark:border-slate-800 dark:bg-slate-950/90 backdrop-blur-xs">
                      <div className="relative flex items-center">
                        ...
                        <input
                          ...
                          className="w-full rounded-lg border border-slate-200 bg-white py-1.5 pl-8 pr-7 text-xs text-slate-900 placeholder:text-slate-400 focus:border-brand focus:outline-hidden dark:border-slate-800 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-500 dark:focus:border-brand"
                      ...
                      filteredOptions.map((opt) => (
                        <ListboxOption
                          ...
                          className={({ focus, selected }) => `
                            ...
                            ${
                              selected
                                ? 'bg-brand/10 text-slate-900 font-semibold border-l-2 border-brand dark:bg-brand/15 dark:text-slate-100 dark:border-brand'
                                : focus
                        ...
                        >
                          {({ selected }) => (
                            <>
                                ...
                                {opt.sigla && opt.value !== '' ? (
                                  <span
                                    className={`shrink-0 rounded px-1.5 py-0.5 text-[10px] font-mono font-bold transition-colors ${
                                      selected
                                        ? 'bg-action text-white dark:bg-action dark:text-white'
                              ...
                              {selected && (
                                <span
                                  className="flex shrink-0 items-center text-brand dark:text-brand"
                                  aria-hidden="true"
                                >
